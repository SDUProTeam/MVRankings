    ...
    <el-col>
      <el-row :span="6" style="padding-left: 64px; padding-right: 64px">
        <el-form
          ...
          size="medium"
        >
            <el-form-item class="form-item" label="电影名">
              <el-input clearable v-model="form.name" style="width: 250px"></el-input>
            </el-form-item>
            <el-form-item class="form-item" label="评分">
              <el-col :span="9">
                <el-form-item>
            ...
            <el-form-item class="form-item" label="主演">
              <el-input clearable v-model="form.casts" style="width: 100px"></el-input>
            </el-form-item>
            <el-form-item class="form-item" label="导演">
      ...
      <el-row style="padding-left: 16px; padding-right: 16px">
        <el-table
          ref="table"
          :data="tableData"
          style="width: 100%;"
        ...
        >
          ...
          <el-table-column label="来源" prop="source" width=150>
            <template slot-scope="scope">
              <el-link
              v-for="(v, i) in scope.row.source" 
              :key="i"
              :href="v.url"
              type="primary">{{srcDict[i]}}&nbsp;</el-link>
            </template>
          </el-table-column>
        </el-table>
...
export default {
  ...
  data() {
    return {
      oldRowJson: null,     
      columns: [
          { field: "name", title: "电影名", width: 200 },
          { field: "rate_douban", title: "豆瓣评分", width: 100, sortable: 'custom' },
          { field: "rate_mtime", title: "时光网评分", width: 120, sortable: 'custom' },
          { field: "rate_maoyan", title: "猫眼评分", width: 100, sortable: 'custom' },
          { field: "time", title: "上映时间", width: 100, sortable: 'custom' },
          { field: "type", title: "类型", width: 100 },
          { field: "casts", title: "主演", width: 300 }, 
  ...
  methods: {
    get() {
      ...
      this.$axios.searchForFusion(_this.form).then(
        response => {
          // 成功回调
          ...
          if (data != null) {
            data.map(i =>{
              for(let src in i.source){
                i['rate_' + src] = i.source[src].rate;
              }
