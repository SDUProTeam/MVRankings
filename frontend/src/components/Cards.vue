  ...
  <div>
    <el-col>
      ...
      <el-row style="padding-left: 48px; padding-right: 48px">
        <dl style="margin-left: 48px; margin-right: 48px">
          <movie-item v-for="(v, i) in tableData" :key='i' :data="v"></movie-item>
        </dl>
...
export default {
  ...
  methods: {
    get() {
      let _this = this;
      _this.qloading = true;
      _this.form['offset'] = this.currentPage * this.form.limit - this.form.limit;
        ...
        response => {
          ...
          if (data != null) {
            data.map(i =>{
              ...
              if (i.rate) {
                i.integer = i.rate.substring(0,2);
                i.fraction = i.rate.substring(2);
              }
              return i;
            });
