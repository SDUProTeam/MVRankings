...
parser.add_argument('--dataset_path', default='datasets/diginetica/',
                    help='dataset directory path: datasets/diginetica/yoochoose1_4/yoochoose1_64')
...
def main():
    ...
    if args.test:
        ckpt = torch.load(args.dataset_path + 'checkpoint.pt')
    ...
    for epoch in range(args.epoch):
        ...
        if mrr > best_mrr:
            ...
            # torch.save(model.state_dict(), args.dataset_path + 'checkpoint.pt')
        print('Epoch {} validation: Recall@{}: {:.4f}, MRR@{}: {:.4f}, Best Epoch: {} \n'
              .format(epoch, args.topk, recall, args.topk, mrr, best_epoch))
...
def trainForEpoch(train_loader, model, optimizer, epoch, num_epochs, criterion, log_aggr=1):
    ...
    print('[TRAIN] epoch %d/%d batch avg %.4f'
          % (epoch + 1, num_epochs, sum_epoch_loss / len(train_loader)))
